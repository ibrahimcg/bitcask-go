...
// and returns the new merged data files.
func (c *Compactor) MergeStaleFiles(dfs []*DataFile) ([]*DataFile, error) {
	...
	if mergedFile.offset == 0 {
		mergedFile.Close()
		os.Remove(filepath.Join(c.directory, fmt.Sprintf("%06d.data", mergeFileId)))
		return nil, nil
	}
...
func (c *Compactor) mergeFile(staleFile *DataFile, mergedFile *DataFile) error {
	...
	for offset < staleFile.offset {
		...
		if !ValidateCrc(rec) {
			log.Printf("warning: CRC mismatch at offset %d in file %d during merge, skipping", offset, staleFile.fileId)
			break
		}
		...
		// A record is live if the KeyDir entry points to this exact location
		meta := c.keyDir.Get(key)
		if meta != nil && meta.fileId == staleFile.fileId && meta.offset == offset {
...
// createHintFile creates a hint file for the given data file.
func (c *Compactor) createHintFile(df *DataFile) error {
	hintPath := filepath.Join(c.directory, fmt.Sprintf("%06d.hint", df.fileId))
...
func (c *Compactor) removeOldFiles() error {
	for _, df := range c.oldFiles {
		path := filepath.Join(c.directory, fmt.Sprintf("%06d.data", df.fileId))
		df.Close()
